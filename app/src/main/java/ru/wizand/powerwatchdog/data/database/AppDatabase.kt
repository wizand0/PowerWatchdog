...
@Database(entities = [PowerEvent::class, PowerSession::class], version = 3, exportSchema = false)
...
abstract class AppDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        // Миграция для добавления колонки message
        private val MIGRATION_2_3 = object : Migration(2, 3) {
            override fun migrate(db: SupportSQLiteDatabase) {
                // SQLite поддерживает добавление колонок через ALTER TABLE
                db.execSQL("ALTER TABLE power_events ADD COLUMN message TEXT")
            }
        }
